...
const OrderHistory =  ({ }) =>{
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await axios.get("http://localhost:3000/history/all");
        setHistory(res.data);
      } catch (err) {
  ...
  const navigationItems = [
    ...
    { name: "Order History", path: "/history", active: true },
              ...
              {history.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {history.map((order) => (
                    <div
                      key={order.id}
                      className="bg-gradient-to-r from-gray-50 to-gray-100 border-2 border-gray-200 hover:border-blue-500 rounded-2xl p-6 shadow-sm transition-all"
                    >
